...
public class NewBeeMallCategoryServiceImpl implements NewBeeMallCategoryService {
    ...
    public List<NewBeeMallIndexCategoryVO> getCategoriesForIndex() {
        ...
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            List<Long> firstLevelCategoryIds = firstLevelCategories.stream().map(GoodsCategory::getCategoryId).collect(Collectors.toList());//每个GoodsCategory对象都映射为CategoryId,转化成一个list合集
            //获取二级分类的数据
